...
public class Loader {
    ...
    //List to keep track of the VAOS and VBOS that we create so we can delete them when we close the game. (memory management).
    private List<Integer> vaos = new ArrayList<Integer>();
    private List<Integer> vbos = new ArrayList<Integer>();
    private List<Integer> textures = new ArrayList<Integer>();  
    
    //Takes in positions about the models vertices, loads data into VAO, return the info about the VAO as a RawModel object.
    public RawModel loadToVAO(float[] positions, float[] textureCoords,float[] normals, int[] indices){
        int vaoID = createVAO();//Creates VAO and stores ID in vaoID.
        bindIndicesBuffer(indices);
        storeDataInAttributeList(0, 3, positions);//stores positional data in attribute list
        storeDataInAttributeList(1, 2, textureCoords);//texture coordinates
        storeDataInAttributeList(2, 3, normals);
        unbindVAO();
        return new RawModel(vaoID, indices.length); //Each vertex has 3 floats. x,y,z. Hence divide by 3.
    }
